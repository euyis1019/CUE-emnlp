...
\begin{itemize}
    ...
    \item We demonstrate \textbf{code-primitive-level mechanistic differences}: Computing emerges as a clear outlier whose failure modes shift qualitatively with arithmetic density; other primitives reveal task-specific bottlenecks only under controlled difficulty increases, showing that aggregate accuracy masks latent mechanistic diversity. Meanwhile, the brewing scaffold remains stable across models and scales while outcome distributions shift with capability, indicating that mechanism and capability are partly separable.
\end{itemize}
