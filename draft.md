...
与这一解释一致，late layers 的 hidden state 还呈现出一个清晰的 sparsity pattern。我们发现 Hoyer sparsity 在最后几层显著升高，而 Overprocessed 样本在这一 late-layer 区域内始终比 non-Overprocessed 样本更 sparse（见 \cref{app:subsec:late_layer_sparsity_overprocessed}）。这说明 overprocessing 不仅表现为行为层面的"先对后错"，还伴随着一种可测的 representation geometry 压缩趋势。
